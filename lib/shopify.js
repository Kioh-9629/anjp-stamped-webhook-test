...
export async function findCustomerByEmail(email) {
  ...
  const res = await shopifyApi.post('2025-04/graphql.json', { query });
...
export async function getCustomerMetafields(customerId) {
  const res = await shopifyApi.get(`2025-04/customers/${customerId}/metafields.json`);
  return res.data.metafields;
}

// 고객 메타필드 저장 또는 덮어쓰기
export async function upsertCustomerMetafield(customerId, namespace, key, value) {
  return await shopifyApi.post(`2025-04/metafields.json`, {
    metafield: {
      namespace,
      key,
      value,
